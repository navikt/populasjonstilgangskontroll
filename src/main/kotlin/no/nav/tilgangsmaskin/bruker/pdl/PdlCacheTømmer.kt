...
@Component
class PdlCacheTømmer(private val cacheOperations: CacheOperations, private val teller: PdlCacheTømmerTeller) {
    ...
    fun listen(hendelse: Personhendelse) {
        ...
        PDL_CACHES.forEach { cache ->
            hendelse.personidenter.forEach { id ->
                if (cacheOperations.delete(id,cache) > 0) {
                    ...
                    log.trace(CONFIDENTIAL,"Slettet nøkkel ${cacheOperations.tilNøkkel(cache, id)} fra cache ${cache.name} etter hendelse av type: {}", id.maskFnr(), Personhendelse::class.simpleName)
