...
class PdlTjeneste(
    ...
    fun medUtvidetFamilie(id: String): Person {
        val person = tilPerson(id, client.person(id, id))
    ...
    @WithSpan
    @Cacheable(cacheNames = [PDL], key = "#root.methodName + ':' + #id")
    fun medFamilie(id: String) = tilPerson(id, client.person(id, id))
    ...
    fun personer(identer: Set<String>): Set<Person> {
        ...
        val fraRest = tilPersoner(client.personer(identer - fraCache.keys)).also {
    ...
    private fun søsken(person: Person): Set<FamilieMedlem> {
        ...
        return buildSet {
            tilPersoner(client.personer(person.foreldre.map { it.brukerId.verdi }.toSet()))
