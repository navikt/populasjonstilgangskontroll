    ...
    private fun sjekk(ansatt: Ansatt, bruker: Bruker, regelSett: RegelSett) =
        with(regelSett) {
            regler.forEachIndexed { index, regel ->
                log.info(CONFIDENTIAL,"[$index] Sjekker regel: '${regel.metadata.kortNavn}' fra regelsett '$tekst' for '${ansatt.ansattId.verdi}/${ansatt.fnr?.verdi}'og '${bruker.brukerId.verdi}'")
