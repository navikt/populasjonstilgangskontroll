...
class HabilitetEgenDataTeller(registry: MeterRegistry, token: Token) :
    AbstractTeller(registry, token, "habilitet.oppslag.egen", "Forsøk på å slå opp egen data")

@Component
class HabilitetFamilieTeller(registry: MeterRegistry, token: Token) :
    AbstractTeller(registry, token, "habilitet.oppslag.familie", "Forsøk på å slå opp familie data")

@Component
class HabilitetSøskenTeller(registry: MeterRegistry, token: Token) :
    AbstractTeller(registry, token, "habilitet.oppslag.søsken", "Forsøk på å slå opp søsken data")

@Component
class HabilitetFellesBarnTeller(registry: MeterRegistry, token: Token) :
    AbstractTeller(registry, token, "habilitet.oppslag.fellesbarn", "Forsøk på å slå opp felles barn data")

@Component
class HabilitetPartnerTeller(registry: MeterRegistry, token: Token) :
    AbstractTeller(registry, token, "habilitet.oppslag.partner", "Forsøk på å slå opp partner data")
