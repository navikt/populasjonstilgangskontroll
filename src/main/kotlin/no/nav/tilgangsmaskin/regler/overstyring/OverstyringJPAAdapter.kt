...
class OverstyringJPAAdapter(private val repository: OverstyringRepository) {
    ...
    fun overstyr(ansattId: String, enhet: String, data: OverstyringData) =
        with(data) {
            repository.save(OverstyringEntity(ansattId, brukerId.verdi, begrunnelse, enhet,gyldigtil.atStartOfDay(systemDefault()).toInstant()))
            Unit
        }
