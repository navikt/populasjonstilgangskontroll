        ...
        Given("role sjekk") {
            When("jwt har claim med tillatt role") {
                Then("returnerer 204 for enkelttilgang") {
                    mockMvc.post("$PROD_BASE_PATH/overstyr") {
                        headers {
                            setBearerAuth(jwt(TEST_AUDIENCE,TEST_ANSATT_ID, mapOf(GROUPS to listOf(OID_ENKELT))))
                        }
            ...
            When("jwt har claim med tillatt og ugyldig role") {
                Then("returnerer 204 for enkelttilgang") {
                    mockMvc.post("$PROD_BASE_PATH/overstyr") {
                        headers {
                            setBearerAuth(jwt(TEST_AUDIENCE, TEST_ANSATT_ID, mapOf(GROUPS to listOf(OID_ENKELT, UTILGJENGELIG))))
                        }
                        contentType = APPLICATION_JSON
                        content = payload
                    }.andExpect {
            ...
            When("jwt har claim med ugyldig role") {
                Then("avvises med 403 med spesifikk melding om manglende ENKELT-rolle") {
                    mockMvc.post("$PROD_BASE_PATH/overstyr") {
                        headers {
                            setBearerAuth(jwt(TEST_AUDIENCE, TEST_ANSATT_ID, mapOf(GROUPS to listOf(UTILGJENGELIG))))
                        }
                        contentType = APPLICATION_JSON
                        content = payload
                    }.andExpect {
