...
class TimeExtensionsTest : BehaviorSpec({
    ...
    Given("isBeforeNow") {
        When("Instant er i fortiden") {
            Then("returneres true") {
                Instant.now().minusSeconds(1).isBeforeNow().shouldBeTrue()
            }
        }
    ...
    Given("månederSidenIdag") {
        When("12 hele måneder siden") {
            Then("gir 12") {
                idag.minusMonths(12).månederSidenIdag() shouldBe 12
            }
        }
        When("24 hele måneder siden") {
            Then("gir 24") {
