...
class RegelMotorTest {
    ...
    fun kode7BrukerKode6Ansatt() {
        assertEquals(fortroligRegel,assertThrows<RegelException> { motor.alleRegler(kode7Bruker, kode6Ansatt) }.regel)
    ...
    fun kode7BrukerVanligAnsatt() {
        assertEquals(fortroligRegel,assertThrows<RegelException> { motor.alleRegler(kode7Bruker, vanligAnsatt) }.regel)
    ...
    fun kode7brukerKode7Ansatt() {
        assertThatCode { motor.alleRegler(kode7Bruker, kode7Ansatt) }.doesNotThrowAnyException()
    }
    @Test
    @DisplayName("Test at kode 6 bruker ikke kan behandles av kode 7 ansatt")
    fun kode6BrukerKode7Ansatt() {
        assertEquals(strengtFortroligRegel,assertThrows<RegelException> { motor.alleRegler(kode6Bruker, kode7Ansatt) }.regel)
    ...
    fun kode6BrukerVanligAnsatt() {
        assertEquals(strengtFortroligRegel,assertThrows<RegelException> { motor.alleRegler(kode6Bruker, vanligAnsatt) }.regel)
    ...
    fun kode6BrukerKode6Ansatt() {
        assertThatCode { motor.alleRegler(kode6Bruker, kode6Ansatt) }.doesNotThrowAnyException()
    }
    @Test
    @DisplayName("Test at vanlig bruker kan behandles av kode 6 ansatt")
    fun vanligBrukertKode6Ansatt() {
        assertThatCode { motor.alleRegler(vanligBruker, kode6Ansatt) }.doesNotThrowAnyException()
    }

    @Test
    @DisplayName("Test at vanlig bruker kan behandles av kode 7 ansatt")
    fun vanligBrukerKode7Ansatt() {
        assertThatCode({ motor.alleRegler(vanligBruker, kode7Ansatt) }).doesNotThrowAnyException()
    }

    @Test
    @DisplayName("Test at vanlig bruker kan behandles av vanlig ansatt")
    fun vanligBrukerVanligAnsatt() {
        assertThatCode { motor.alleRegler(vanligBruker, vanligAnsatt) }.doesNotThrowAnyException()
    }

    @Test
    @DisplayName("Test at egen ansatt bruker kan behandles av egen ansatt ansatt")
    fun egenAnsattBrukerEgenAnsatt() {
        assertThatCode { motor.alleRegler(ansattBruker, egenAnsatt) }.doesNotThrowAnyException()
    }

    @Test
    @DisplayName("Test at egen ansatt bruker ikke kan behandles av kode7 ansatt")
    fun ansattBrukerKode7ansatt() {
        assertEquals(egenAnsattRegel, assertThrows<RegelException> { motor.alleRegler(ansattBruker, kode7Ansatt) }.regel)
    ...
    fun ansattBrukerKode6Ansatt() {
        assertEquals(egenAnsattRegel,assertThrows<RegelException> { motor.alleRegler(ansattBruker, kode6Ansatt) }.regel)
    ...
    fun ansattBrukerVanligAnsatt() {
        assertEquals(egenAnsattRegel,assertThrows<RegelException> { motor.alleRegler(ansattBruker, vanligAnsatt) }.regel)
    ...
    fun ansattKode6BrukerEgenAnsatt() {
        assertEquals(strengtFortroligRegel,assertThrows<RegelException> { motor.alleRegler(ansattKode6Bruker, egenAnsatt) }.regel)

    }
    @Test
    @DisplayName("Test at egen ansatt bruker med kode 7 ikke kan behandles av egen ansatt")
    fun ansattKode7BrukerEgenAnsatt() {
        assertEquals(fortroligRegel,assertThrows<RegelException> { motor.alleRegler(ansattKode7Bruker, egenAnsatt) }.regel)
    ...
    fun egenAnsattBrukerKode7Ansatt() {
        assertThatCode { motor.alleRegler(ansattKode7Bruker, kode7EgenAnsatt) }.doesNotThrowAnyException()
    }
    @Test
    ...
    fun ansattKode6BrukerKode6Ansatt() {
        assertThatCode { motor.alleRegler(ansattKode6Bruker, kode6EgenAnsatt) }.doesNotThrowAnyException()
    }
    @Test
    @DisplayName("Test at egen ansatt bruker med kode 6 ikke kan behandles av kode 7 ansatt")
    fun ansattKode6BrukerKode7Ansatt() {
        assertEquals(strengtFortroligRegel,assertThrows<RegelException> {motor.alleRegler(ansattKode6Bruker, kode7Ansatt) }.regel)
    ...
    fun brukerMedManglendeGeografiskTilknytningAnsattMedSamme() {
        assertThatCode { motor.alleRegler(ukjentBostedBruker, udefinertGeoAnsatt) }.doesNotThrowAnyException()
    }
    ...
    fun brukerMedManglendeGeografiskTilknytningAnsattUtenSammeRole() {
        assertEquals(ukjentBostedGeoRegel,assertThrows<RegelException> {motor.alleRegler(ukjentBostedBruker, vanligAnsatt)}.regel)
    ...
    fun geoUtlandGruppe() {
        assertThatCode {
            motor.alleRegler(geoUtlandBruker, geoUtlandAnsatt)
    ...
    fun geoUtlandGruppeUtenSammeRolle() {
        assertEquals(geoUtlandRegel,assertThrows<RegelException> {motor.alleRegler(geoUtlandBruker, vanligAnsatt)  }.regel)
    ...
    fun geoNorgeNasjonal() {
        assertThatCode {
            motor.alleRegler(vanligBruker, nasjonalAnsatt)
        }.doesNotThrowAnyException()
    }
    ...
    fun geoEnhetLik() {
        assertThatCode {
            motor.alleRegler(enhetBruker, enhetAnsatt)
        }.doesNotThrowAnyException()
    }
    ...
    fun geoEnhetForskjellig() {
        assertEquals(geoNorgeRegel,assertThrows<RegelException> {motor.alleRegler(enhetBruker1, enhetAnsatt)  }.regel)
